...
import { USDTcontractAddress, USDTcontractABI, CCcontractAddress, rechargeABI, poolAccount } from "./constants";
import { storeToRefs } from 'pinia'
const store = useAddrStore()
const { address } = storeToRefs(store)


export async function expcoinaddr() {
...
export async function createCCContract() {
  const provider = await getProvider();
  const signer = await provider.getSigner();
  const TransactionsContract = new ethers.Contract(CCcontractAddress, USDTcontractABI, signer);
...
export async function sign() {
  ...
  let result = false
  try {
    result = await signer.signMessage(`Authorize Login`, address.value, 'Authorize');
  } catch (error) {
    result = false
  }
  return result;
...
export async function allowance(cointype) {
  let status = null
  try {
    let MyContract = cointype == 1 ? await createUSDTContract() : await createCCContract()
...
export async function ApproveCoin(cointype, cb) {
  ...
  from = cointype == 1 ? await createUSDTContract() : await createCCContract()
